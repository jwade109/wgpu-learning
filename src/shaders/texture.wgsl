...
fn vs_main(vertex: Vertex) -> VertexShaderOut {
    ...
    out.position = proj_mat * transform * vec4<f32>(vertex.position, 1.0);
...
fn fs_main(in: VertexShaderOut) -> FragmentShaderOut {
    ...
    let y = sin(in.tex_coord.x * 7.0 + params.time) * 0.1 + 0.4;
    let d = abs(in.tex_coord.y - y);

    let s = 1.0 - step(0.02, d) * 0.5 - step(0.05, d) * 0.5;
    let c_s = vec4<f32>(s, s, s, 1.0);
